...
- OCR：服务端内置 Python 脚本调用 PaddleOCR v2 Jobs 异步接口
- PPT：PptxGenJS
...
- PaddleOCR SSL EOF 错误会进行诊断提示，并保留可信私有端点下的 `PADDLEOCR_DOC_PARSING_INSECURE_TLS=1` 兜底开关。
...
- `paddleApiUrl`：可选，覆盖 PaddleOCR Jobs endpoint
